...
const Skill = (props: Props) => {
  return (
    <div className=" group relative cursor-pointer flex">
      <div className="rounded-full border border-slate-320 bg-slate-800 dark:bg-indigo-100 object-cover filter group-hover:grayscale transition duration-500 ease-in-out p-6 md:p-10 text-slate-200 dark:text-slate-900">
      ...
      <div className="absolute opacity-0 group-hover:opacity-90 transition duration-320 ease-in-out dark:group-hover:bg-indigo-100 group-hover:bg-blue-900 w-full h-full rounded-full">
        <div className="flex flex-col items-center justify-center h-full">
          <p className="text-sm md:text-lg font-medium text-slate-200 dark:text-red-600 opacity-100">{props.name}</p>
          <p className="text-base md:text-2xl font-bold text-yellow-400 dark:text-black opacity-100">{props.value}</p>
        </div>
      </div>
    </div>
  );
